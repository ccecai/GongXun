...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define PID_DEFAULT_PRECISION 0.000f	 //控制精度，当目标速度与实际速度小于此值时，认为没有误差，使PID更稳定
#define PID_DEFAULT_ERRALL_MAX 20000		 //控制ERR_ALL最大值，否则ERR_ALL最大值过大，会使PID反应慢，不稳定，积分限幅
#define PID_DEFAULT_OUTPUT_MAX 2000		 //输出限幅, 相当于没限制幅度
#define PID_DEFAULT_OUTPUT_STEP_MAX 3000 //输出微分限幅，防止反复正反电机，损害大

#define PID_DEFAULT_KP 450.0f
#define PID_DEFAULT_KI 5.0f
#define PID_DEFAULT_KD 0.0f


/* Global param ------------------------------------------------------------*/
SPEEDC_SpeedsTypeDef pid_speeds_t; //用来输出速度
PID_PramTypeDef speed_pid_t;
/* Private functions --------------------------------------------------------*/
/**
...
float PID_UpdateTargetNow(PID_PramTypeDef *WhichPID)
{
	...
	else if (WhichPID->PID_RunState == PID_RAMP)
	{
...
/**
  ...
  * @retval None
  */
void PID_SetTagretWithNormal(PID_PramTypeDef *WhichPID, float Tartget)
{
...
{
	PID_SetTagretWithNormal(&speed_pid_t, target);
}

void PID_NormalLoop(void)
{
	PID_GetOutput(&speed_pid_t, pid_speeds_t.ecd_speed1);
	MOTER_SetCurrent(speed_pid_t.out_now, 0, 0, 0);
}
/**
  * @brief 初始化PID为默认参数
...
void PID_DeInit(PID_PramTypeDef *WhichPID)
{
	...
	WhichPID->pid_err_max = PID_DEFAULT_ERRALL_MAX;
	WhichPID->pid_out_max = PID_DEFAULT_OUTPUT_MAX;
	WhichPID->pid_outstep_max = PID_DEFAULT_OUTPUT_STEP_MAX;
}
...
void PID_NormalDebug(void)
{
	...
	while (1)
	{
		if (pid_speeds_t.speed_update_times >= 1) //1次打印一次
		{
			...
			//输出单个轮子的目标值、当前速度
			printf("v1,target:%d,%.2f,%.2f\n\r", pid_speeds_t.ecd_speed1, speed_pid_t.target_now, speed_pid_t.out_now);
